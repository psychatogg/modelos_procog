...
\end{align*}
\\
b) Queremos conocer la tasa de cambio (pendiente) de la velocidad de olvido. Para ello empleamos la 2º derivada: \\
\begin{align*}
&\text{Modelo exponencial} \\
&P_0^{\prime}(t)=0,15 \cdot e^{-0,25 t} \\
&P_0^{\prime \prime}(t)=0,15 \cdot e^{-0,25 t} \cdot(-0,25) \\
&P_0^{\prime \prime}(t)=-0,04 \cdot e^{-0,25 t} \\
&\\
&\text{Modelo potencial} \\
&P_0^{\prime}(t)=0,43 \cdot(1+t)^{-1,68} \\
&P_0^{\prime \prime}(t)=0,43 \cdot(-1,68) \cdot(1+t)^{-2,68} \\
&P_0^{\prime \prime}(t)=-0,72 \cdot(1+t)^{-2,68} \\
\end{align*}
...
\\
Dada la escala tanto del dominio como de la imagen con la que trabajamos en este contexto de psicología básica, opto por localizar los puntos de corte entre ambas $P_O^{\prime \prime}$ por medio de resolución gráfica. \\
...
&t_2 = 22,09 \\
\end{align*}
Por lo tanto, la $P_O^{\prime} \text {exponencial} > P_O^{\prime} \text {potencial} \ \forall t \in (2,83 , 22,09)$\\
\vspace{0.5cm}
\end{document}