...
&\text {Aplicamos logaritmo de la potencia y extraemos factor común del productorio}\\
&f(x)=x^2 \ln \left(\prod_{i=1}^n a_i\right)\\
&\operatorname{Dom} f=\left\{\mathbb{R}: \prod_{i=1}^{n} a_i^{x^2}>0\right\}\\
&f^{\prime}(x)=2 x \cdot \ln \left(\prod_{i=1}^n a_i\right)\\
&\operatorname{Dom} f^{\prime}=\left\{\mathbb{R}: \prod_{i=1}^n a_i^{x^2}>0\right\} \\
\\
&1.5 - \\
&f(x)=\sum_{i=1}^k\left(a_i+b_i \cdot \ln (x)\right)^2 \\
&\operatorname{Dom} f=\{\mathbb{R}: x>0\} \\
