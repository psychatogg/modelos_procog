...
b) $f(x) = |x|$ cumple con la definición de función, ya que el primer miembro de cada par ordenado es diferente en su definición por extensión. Sin embargo, en su inversa, el primer miembro de cada par ordenado es igual:
$$
\begin{aligned}
&x = \pm y \\
&f^{-1} = \pm y
